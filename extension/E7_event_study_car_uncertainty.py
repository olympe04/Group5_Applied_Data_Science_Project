...
#   Outputs: data_clean/ecb_event_study_car_uncertainty.csv (CAR table)
#            data_clean/ecb_uncertainty_with_car.csv (merged dataset).
...
def resolve_paths(project_root: Path) -> tuple[Path, Path, Path]:
    ...
    return raw / "^SX5E data.xlsx", clean / "ecb_uncertainty_lm.csv", clean
...
def export_car_table(out: pd.DataFrame, clean_dir: Path) -> Path:
    p = clean_dir / "ecb_event_study_car_uncertainty.csv"
    out_export = out.copy()
...
def merge_with_uncertainty(ecb: pd.DataFrame, out: pd.DataFrame, clean_dir: Path) -> Path:
    p = clean_dir / "ecb_uncertainty_with_car.csv"
...
def main() -> None:
    ...
    sx5e_path, unc_path, clean_dir = resolve_paths(project_root)
    ...
    p_merged = merge_with_uncertainty(ecb, out, clean_dir)
